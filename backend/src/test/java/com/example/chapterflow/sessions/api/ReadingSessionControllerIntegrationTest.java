...
import com.example.chapterflow.shared.security.UserAuthenticationToken;
...
class ReadingSessionControllerIntegrationTest {
        ...
        void testStartSession_Success() throws Exception {
                var request = new com.example.chapterflow.sessions.api.dto.StartSessionRequest(testBook.getId());
        ...
        void testStopSession_Success() throws Exception {
                mockMvc.perform(post("/api/sessions/start")
                                ...
                                .content(objectMapper.writeValueAsString(
                                                new com.example.chapterflow.sessions.api.dto.StartSessionRequest(
                                                                testBook.getId()))))
                                .andExpect(status().isCreated());

                var stopRequest = new com.example.chapterflow.sessions.api.dto.StopSessionRequest(null, null);
        ...
        void testStopSession_WithEndTimeAndEndPage_ShouldPersistValues() throws Exception {
                ...
                var stopRequest = new com.example.chapterflow.sessions.api.dto.StopSessionRequest(fixedEndTime, 50);
                mockMvc.perform(post("/api/sessions/stop")
                                .with(jwtForUser()).with(csrf())
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(objectMapper.writeValueAsString(stopRequest)))
